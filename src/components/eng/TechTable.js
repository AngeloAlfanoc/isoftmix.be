...
class TechTable extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            location: window
                .location
                .pathname
                .slice(14, window.location.pathname.length)
                .replace('_', ' ')
        };
    }
    componentDidMount() {
    ...
    buildTablesHead = (data) => {
        return data.map((item, i) => {
            if (this.state.location === item.acf.product_name_eng.toLowerCase() ) {
                return (<th style={{color:'#FFBF37'}} key={i} scope="col">{item.acf.product_name_eng}</th>)
            }
            else{
                return (<th key={i} scope="col">{item.acf.product_name_eng}</th>)
            }              
        })
    }
    ...
    buildTablesBodyVermogenVanDeLamp = (data) => {
        return data.map((item, i) => {
            if (this.state.location === item.acf.product_name_eng.toLowerCase() ) {
                return (  <td style={{fontWeight:'600',borderBottom:1+'px #d1d1d1 solid'}} key={i}>{item.acf.vermogen_van_de_lamp_in_w}</td>)
            } else{
                return (  <td key={i} style={{borderBottom:1+'px #d1d1d1 solid'}}>{item.acf.vermogen_van_de_lamp_in_w}</td>)
            }
             })
    }
    render() {
        ...
        if (isLoaded===true && isLoadedProperties===true) {
            ...
            <section style={{overflowX:'auto'}} id="techTable" className="container mt-5">
                <table className="table mt-5">
                    <thead>
                        <tr className="w-100">
                            <th scope="col" style={{fontWeight:500, fontStyle:'italic'}}>{properties.technische_tabel_eng}</th>
                            {this.buildTablesHead(data)}
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
             <th scope="row" style={{fontWeight:300}}>{properties.capaciteit_eng}</th>
                            {this.buildTablesBodyCapaciteit(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.nominaal_eng}</th>
                            {this.buildTablesBodyNominaal(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.max_debiet_eng}</th>
                            {this.buildTablesBodyMaxDebiet(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.volume_in_liters_eng}</th>
                            {this.buildTablesBodyiMIX(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.zoutvraag_in_kilo_eng}</th>
                            {this.buildTablesBodyZoutvraag(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.waterverbruik_in_m_eng}</th>
                            {this.buildTablesBodyWaterverbruik(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.werkdruk_min_max_in_bar_eng}</th>
                            {this.buildTablesBodyWerkdruikMinMax(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.werktemperatuur_in_graden_eng}</th>
                            {this.buildTablesBodyWerkTempInC(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.max_temperatuur_in_graden_eng}</th>
                            {this.buildTablesBodyMaxTempInC(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.aansluitingen_in_out_eng}</th>
                            {this.buildTablesBodyAansluitingInOut(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.elektrische_aansluiting_eng}</th>
                            {this.buildTablesBodyElekAan(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:500, fontStyle:'italic',borderRight:0}} className="pt-4">{properties.pekelbak_eng}</th>
                        ...
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.diameter_x_hoogte_in_mm_eng}</th>
                            {this.buildTablesBodyDiameterHoogte(data)}
                        </tr>

                        <tr>
                            <th scope="row" style={{fontWeight:500, fontStyle:'italic',borderRight:0}}>{properties.afmetingen_per_fles_in_mm_eng}</th>
                            
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.breedte_eng}</th>
                            {this.buildTablesBodyBreedte(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.diepte_eng}</th>
                            {this.buildTablesBodyDiepte(data)}
                        </tr>
                        <tr>
                            <th scope="row" style={{fontWeight:500, fontStyle:'italic',borderRight:0}}>{properties.optie_uv_eng}</th>
                            {this.buildTablesBodyOptieUV(data)}
                        </tr>
                        <tr>
                            <th scope="row" style={{fontWeight:300,borderBottom:1+'px #d1d1d1 solid'}}>{properties.vermogen_van_de_lamp_in_watt_eng}</th>
