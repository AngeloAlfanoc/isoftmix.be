...
class Nav extends React.Component {
    ...
    fetchMenu = async() => {
        await axios
            .get(frontConfig.mainRestRef + frontConfig.mainJsonStream + frontConfig.getPrimaryMenuEng)
            .then((result) => {
    ...
    buildListedItems = (p) => {
        ...
        .items
        .map((item, i) => {
            if (item.url === this.state.location && item.url !== routesObject.routesENG.contact) {
                return (
                    <li key={i}>
                        <a className="active" id={item.title} href={item.url}>{item.title}</a>
                    </li>
                )
            } else if (item.url === routesObject.routesENG.products && this.state.location.slice(0, 19) === item.url + '/isoft') {
                return (
                    <li key={i}>
                        <a className="active" id={item.title} href={item.url}>{item.title}</a>
                    </li>
                )
            } else if (item.url === routesObject.routesENG.contact && item.url === this.state.location) {
                return <li key={i}>
                    <a
            ...
            } else if (item.url === routesObject.routesENG.contact) {
                return <li key={i}>
    ...
    handleScroll = () => {if (document.body.offsetHeight > 2000) { const currentScrollPos = window.pageYOffset;
        const visible = 220> window.pageYOffset;
    ...
    render() {
        ...
        if (error) {
            ...
            })}> <div className={classnames("middle container d-flex align-items-center justify-content-between border-black ", {
