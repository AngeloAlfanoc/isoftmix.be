...
class TechTable extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            location: window
                .location
                .pathname
                .slice(11, window.location.pathname.length)
                .replace('_', ' ')
        };
    }
    componentDidMount() {
    ...
    buildTablesHead = (data) => {
        return data.map((item, i) => {
            if (this.state.location === item.acf.product_name_nl.toLowerCase()) {
                return (<th style={{color:'#FFBF37'}} key={i} scope="col">{item.acf.product_name_nl}</th>)
            }
            else{
                return (<th key={i} scope="col">{item.acf.product_name_nl}</th>)
            }              
        })
    }
        ...
        if (isLoaded===true && isLoadedProperties===true) {
            ...
            <section style={{overflowX:'auto'}} id="techTable" className="container mt-5">
                <table className="table mt-5">
                    <thead>
                        <tr className="w-100">
                            <th scope="col" style={{fontWeight:500, fontStyle:'italic'}}>{properties.technische_tabel_nl}</th>
                            {this.buildTablesHead(data)}
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
             <th scope="row" style={{fontWeight:300}}>{properties.capaciteit_nl}</th>
                            {this.buildTablesBodyCapaciteit(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.nominaal_nl}</th>
                            {this.buildTablesBodyNominaal(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.max_debiet_nl}</th>
                            {this.buildTablesBodyMaxDebiet(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.volume_in_liters_nl}</th>
                            {this.buildTablesBodyiMIX(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.zoutvraag_in_kilo_nl}</th>
                            {this.buildTablesBodyZoutvraag(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.waterverbruik_in_m_nl}</th>
                            {this.buildTablesBodyWaterverbruik(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.werkdruk_min_max_in_bar_nl}</th>
                            {this.buildTablesBodyWerkdruikMinMax(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.werktemperatuur_in_graden_nl}</th>
                            {this.buildTablesBodyWerkTempInC(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.max_temperatuur_in_graden_nl}</th>
                            {this.buildTablesBodyMaxTempInC(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.aansluitingen_in_out_nl}</th>
                            {this.buildTablesBodyAansluitingInOut(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.elektrische_aansluiting_nl}</th>
                            {this.buildTablesBodyElekAan(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:500, fontStyle:'italic',borderRight:0}} className="pt-4">{properties.pekelbak_nl}</th>
                        ...
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.diameter_x_hoogte_in_mm_nl}</th>
                            {this.buildTablesBodyDiameterHoogte(data)}
                        </tr>

                        <tr>
                            <th scope="row" style={{fontWeight:500, fontStyle:'italic',borderRight:0}}>{properties.afmetingen_per_fles_in_mm_nl_1}</th>
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.breedte_nl}</th>
                            {this.buildTablesBodyBreedte(data)}
                        </tr>
                        <tr>
                            <th scope="row"  style={{fontWeight:300}}>{properties.diepte_nl}</th>
                            {this.buildTablesBodyDiepte(data)}
                        </tr>
                        <tr>
                            <th scope="row" style={{fontWeight:500, fontStyle:'italic',borderRight:0}}>{properties.optie_uv_nl}</th>
                            {this.buildTablesBodyOptieUV(data)}
                        </tr>
                        <tr>
                            <th scope="row" style={{fontWeight:300,borderBottom:1+'px #d1d1d1 solid'}}>{properties.vermogen_van_de_lamp_in_watt_nl}</th>
