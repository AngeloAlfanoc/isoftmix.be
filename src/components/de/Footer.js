...
class Footer extends React.Component {
    ...
    buildProductMenu = (p) => {
        ...
        if (isLoadedMenu3 === true) {
            return p.map((item, i) => {
                if (i < 5) {
                    return (<li className="d-flex align-items-center" key={i}>
                            <FontAwesomeIcon style={{fontSize: '0.8em',color: 'white'}} icon={faChevronRight}/><a className="pl-1" href={'/de/produkte/' + item.acf.permalink}>{item.acf.product_name_nl}</a>
                        </li>
                    )
                }
                else{return ''}
