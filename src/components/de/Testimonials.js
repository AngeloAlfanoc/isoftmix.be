...
class Referenzen extends React.Component {
    ...
    buildForm = () => {
        ...
        <form className="d-flex flex-column align-items-center" onSubmit={this.submitForm.bind(this)} method='POST'>
            <input className="form-control w-100" type="text" required placeholder="Dein Name" aria-describedby="nameHelp"  value={this.state.user} onChange={this.onUserChange.bind(this)}></input>
            <input className="form-control w-100" type="text" required placeholder="Deine E-Mail" aria-describedby="emailHelp"  value={this.state.email} onChange={this.onEmailChange.bind(this)}></input>
            <textarea maxLength='400'  className="form-control w-100" rows="5" required placeholder='Teile deine Erfahrung?'   value={this.state.message} onChange={this.onMessageChange.bind(this)}></textarea>
        ...
        return (<>  
                <div className="">
                    <div className="d-flex flex-row align-items-center py-5">
                        <span className="mr-3">
                            <a href={routesObject.routesDE.testimonials}><h2>Referenzen</h2></a>
                        </span>
                        <span>
                            <a href={routesObject.routesDE.testimonials}><FontAwesomeIcon style={{ fontSize: '1.4em', color: '#FFBF37' }} icon={faChevronRight}/></a>
                        </span>
                    </div>
                    <Slider {...settings}>
                    {p.map((item, i) => {
                    ...
                    <div className="py-4"  key={i}>
                            ...
                            <p onClick={event => window.location.href = '/de/Referenzen'} style={{maxWidth:60+'%', maxHeight:250,textAlign:'center', fontSize:'1em'}}>{itemText}</p>
...
Teile deine Erfahrung? </button>}modal closeOnDocumentClick>
                            {this.buildForm()}
                        </Popup>
        ...
        <section className="testimonials" style={{paddingBottom: 100+'px'}}>
            <ScrollAnimation animateIn="fadeIn">
                ...
                <span className="mr-3">
                    ...
                    <p className="p-2">Es sieht so aus, als gäbe es noch keine Testimonials. <br/>
Klicken Sie <a href="/de/referenzen">hier,</a> um eine hinzuzufügen</p></> : this.buildTestimonial(data)} 
                </div>
            </ScrollAnimation>
        </section>);
        }
      ...
      <ScrollAnimation animateIn="fadeIn" animateOnce={true}>
                ...
                {data == 0? <>                    <div className="d-flex flex-row align-items-center justify-content-between flex-wrap">
                        ...
                        <div className="justify-content-end">
                        <Popup trigger={<button className="btn"> Teile deine Erfahrung?</button>}modal closeOnDocumentClick>
                            {this.buildForm()}
                        </Popup>
                        
                        </div>
                    </div>
                    <p className="p-2">Es sieht so aus, als gäbe es noch keine Testimonials. </p></> : this.buildTestimonialStandalone(data)} 
                </div>
            </ScrollAnimation>
        </section>);
        } else{
