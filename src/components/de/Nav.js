...
class Nav extends React.Component {
    ...
    buildListedItems = (p) => {
        ...
        .map((item, i) => {
            ...
            if (item.url === this.state.location && item.url !== routesObject.routesDE.contact) {
                return (
                    <li key={i}>
                        <a className="active" id={item.title} href={item.url}>{item.title}</a>
                    </li>
                )
            } else if (item.url  === routesObject.routesDE.products && this.state.location.slice(0, 18) === item.url + '/isoft') {
                return (
                    <li key={i}>
                        <a className="active" id={item.title} href={item.url}>{item.title}</a>
                    </li>
                )
            } else if (item.url === routesObject.routesDE.contact && item.url === this.state.location) {
                return <li key={i}>
                    <a
            ...
            } else if (item.url === routesObject.routesDE.contact) {
                return <li key={i}>
