...
class Testimonials extends React.Component {
    ...
    submitForm = async (e) => {
        ...
        await axios({
            ...
            window.location.href = routesObject.routesFR.testimonials
       }, (error) => {
           console.error(error)
        })
    }
    ...
    buildForm = () => {
        ...
        <form className="d-flex flex-column align-items-center" onSubmit={this.submitForm.bind(this)} method='POST'>
            <input placeholder="Votre nom" className="form-control w-100" type="text" required aria-describedby="nameHelp"  value={this.state.user} onChange={this.onUserChange.bind(this)}></input>
            <input placeholder="Votre email" className="form-control w-100" type="text" required aria-describedby="emailHelp"  value={this.state.email} onChange={this.onEmailChange.bind(this)}></input>
            <textarea placeholder='Votre message' maxLength='400'  className="form-control w-100" rows="5" required   value={this.state.message} onChange={this.onMessageChange.bind(this)}></textarea>
    ...
    <div className="">
                    <div className="d-flex flex-row align-items-center py-5">
                        <span className="mr-3">
                        <a href={routesObject.routesFR.testimonials}><h2>Témoignages</h2></a>
                        </span>
                        <span>
                            <a href={routesObject.routesFR.testimonials}><FontAwesomeIcon style={{ fontSize: '1.4em', color: '#FFBF37' }} icon={faChevronRight}/></a>
                        </span>
                    </div>
                    <Slider {...settings}>
                    {p.map((item, i) => {
                    ...
                    <div className="py-4"  key={i}>
                            ...
                            <p onClick={event => window.location.href = routesObject.routesFR.testimonials} style={{maxWidth:60+'%', maxHeight:250,textAlign:'center', fontSize:'1em'}}>{itemText}</p>
        ...
        <>  
                <div>
                    <div className="d-flex flex-row align-items-center justify-content-between flex-wrap">
                        ...
                        <Popup trigger={<button className="btn"> Partagez votre expérience? </button>}modal closeOnDocumentClick>
                            {this.buildForm()}
                        </Popup>
        ...
        <section className="testimonials" style={{paddingBottom: 250+'px'}}>
            <ScrollAnimation animateIn="fadeIn" animateOnce={true}>
                ...
                <span className="mr-3">
                    ...
                    <p className="p-2">
Il semble qu'il n'y ait pas encore de témoignages .. <br/>Cliquez <a href={routesObject.routesFR.testimonials}>ici </a>pour en ajouter un</p></> : this.buildTestimonial(data)} 
                </div>
            </ScrollAnimation>
        </section>);
        }
        ...
        <section className="testimonials contact" style={{paddingBottom: 250+'px'}}>
            <ScrollAnimation animateIn="fadeIn">
                ...
                {data == 0? <>                    <div className="d-flex flex-row align-items-center justify-content-between flex-wrap">
                    ...
                    <p className="p-2">
Il semble qu'il n'y ait pas encore de témoignages .. </p></> : this.buildTestimonialStandalone(data)} 
                </div>
            </ScrollAnimation>
        </section>);
        } else{
