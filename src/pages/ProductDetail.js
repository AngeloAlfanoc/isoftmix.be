        ...
        if (isLoaded === true) {
            return p.map((item, i) => {
                ...
                if (location === productNeeded) {
                    ...
                    let sanitizeSEOTAG = item.acf.descr_nl.replace(/(<([^>]+)>)/ig,"");
                    return ( <> <section key={i} className="product-detail container mb-5">
                            <Helmet>
                                <title>{item.title.rendered}</title>
                                <meta name="description" content={sanitizeSEOTAG}/>
                                <meta name="keywords" content={item.acf.zoekwoorden}/>
                            </Helmet>
